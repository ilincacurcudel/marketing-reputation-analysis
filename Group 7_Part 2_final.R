...
# Load data for all parts of assignment
data <- read_xlsx("sv.xlsx")
data1 <- as.data.frame(data)
sp500<-read.csv("HistoricalData_1678716045367.csv")
data2<-read.csv("realized_volatility.csv")
...
var(demeaned_y)
summary(demeaned_y)
plot(demeaned_y,type = "l", ylab="", xlab = "",main = "Demeaned log return S&P500 2018-2023")
...
##graph of smoothed h_t (alpha_t) and x_t
plot(x, type = "l", ylab="", xlab = "", main="Smoothed data")
lines(alpha_t, col="red")
legend("bottomleft", legend=c("Transformed data", "Smoothed data"),
       col=c("black", "red"), lty=1:2, cex=0.7)
...
H_tt <- a_t - ksi_hat
# smoothed expectation H_t | Y_n
H_nn <- alpha_t - ksi_hat

#plot filtered H_t nd smoothed H_t
plot(H_tt, type="l", ylab="", xlab = "", main="Filtered and smoothed H")
lines(H_nn, col="red")
legend("bottomleft", legend=c("Filtered H", "Smoothed H"),
       col=c("black", "red"), lty=1:2, cex=0.6)

####### extension e########
sp500_data<-data2[which(data2$Symbol == ".SPX"),]
y_2<-diff(log(sp500_data$close_price))
x_2<-log((y_2-mean(y_2))^2)
rv_data<-log(sp500_data$rk_parzen)
rv<-rv_data[-1]


##########augmentation #########
x1<-x_2[1:(length(x_2)-1)]
x2<-x_2[2:length(x_2)]
phi <- cov(x1,x2)/(var(x_2)-pi^2/2)  
sigma2_eta <- (1 - phi^2)*(var(x_2) - pi^2/2)
omega <- (1 - phi)*(mean(x_2) + 1.27)
par_ini<-c(phi,omega,sigma2_eta)

est <- optim(par = par_ini, fn=loglikelihood, x = x_2, method = "BFGS")
phi_hat <- exp(est$par[1])/(1+exp(est$par[1]))
omega_hat <- est$par[2]
sigma2_eta_hat <- exp(est$par[3])
...
#output of kalman filter
output_x_2<-kalman_filter(x_2, phi_hat, omega_hat, sigma2_eta_hat)
output_rv<-kalman_filter(rv-1.27,phi_hat, omega_hat, sigma2_eta_hat)
f_t<-output_rv$f_t
x_t<-output_rv$v_t
v_t<-output_x_2$v_t
sum_1<-0
for (i in 1:length(x_t)){
  sum_1<-sum_1+(x_t[i]/f_t[i]*v_t[i])
}

sum_2<-0
for (i in 1:length(x_t)){
  sum_2<-sum_2+(x_t[i]/f_t[i]*x_t[i])
}
beta_hat<-sum_1/sum_2
...
# PLOT I 
plot(H_hat, type="l", ylab="", xlab = "", main="Bootstrap particle filter estimates: initial data", ylim=c(-1, 1))
lines(H_t, col="red")
...
# H_tt: filtered QML estimates calculated in e based on the stock index data

# PLOT II
plot(H_hat2, type="l", ylab="", xlab = "", main="Bootstrap particle filter estimates: stock index data", ylim=c(-4, 4))
lines(H_tt, col="red")
legend("bottomleft", legend=c("Filtered h", "Filtered QML h"),
       col=c("black", "red"), lty=1:2, cex=0.6)
